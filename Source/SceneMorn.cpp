...
void SceneMorn::Init()
{
	...
	//WALLS
	meshList[GEO_WALL] = MeshBuilder::GenerateOBJ("wall", "OBJ//bigwall.obj");
	...
	//BOMB
	meshList[GEO_BOMB] = MeshBuilder::GenerateOBJ("Bomb", "OBJ//cubebombs.obj");
	meshList[GEO_BOMB]->textureID = LoadTGA("Image//cubebomb.tga");
	meshList[GEO_BOMB]->material.kAmbient.Set(0.5f, 0.5f, 0.5f);
	meshList[GEO_BOMB]->material.kDiffuse.Set(0.6f, 0.6f, 0.6f);
	meshList[GEO_BOMB]->material.kSpecular.Set(0.3f, 0.3f, 0.3f);
	meshList[GEO_BOMB]->material.kShininess = 1.f;

	meshList[GEO_TENT] = MeshBuilder::GenerateOBJ("Bomb", "OBJ//tent2.obj");
	meshList[GEO_TENT]->textureID = LoadTGA("Image//tent.tga");

	//WATCHTOWER
	meshList[GEO_WATCHTOWER] = MeshBuilder::GenerateOBJ("Bomb", "OBJ//watchtower.obj");
	meshList[GEO_WATCHTOWER]->textureID = LoadTGA("Image//watchtower.tga");
	meshList[GEO_WATCHTOWER]->material.kAmbient.Set(0.5f, 0.5f, 0.5f);
	meshList[GEO_WATCHTOWER]->material.kDiffuse.Set(0.6f, 0.6f, 0.6f);
	meshList[GEO_WATCHTOWER]->material.kSpecular.Set(0.3f, 0.3f, 0.3f);
	meshList[GEO_WATCHTOWER]->material.kShininess = 1.f;
	meshList[GEO_WATCHWALL] = MeshBuilder::GenerateOBJ("cube", "OBJ//Cube.obj");
	meshList[GEO_WATCHWALL]->textureID = LoadTGA("Image//watchtower.tga");

	meshList[GEO_GUY] = MeshBuilder::GenerateOBJ("Man", "OBJ//doorman.obj");
	meshList[GEO_GUY]->textureID = LoadTGA("Image//doorman.tga");
...
void SceneMorn::Update(double dt)
{
	...
	if (triggeredBomb == true && loopCount == 0)
	{
		bombTime = elaspeTime + 10.0f;
		++loopCount;
	}
	if (elaspeTime > bombTime && triggeredBomb == true)
	{
		scaleAll += (float)(500 * dt);
		bombExploded = true;
	}
	...
	if (bombExploded == false)
	{
		if (controlGuy == true)
		{
			camera.position = Vector3(500, 300, 500);
			...
			if (Application::IsKeyPressed('Q'))
			{
				...
				camera.target = Vector3(guyPosX, 10, guyPosZ);
			}
		}
		else
		{
...
void SceneMorn::Render()
{
	...
	//Front wall
	modelStack.PushMatrix();
	modelStack.Translate(-500, 40, -800);
	...
	//TUNNEL
	modelStack.PushMatrix();
	modelStack.Translate(600, -11, -730);
	...
	{
		modelStack.PushMatrix();
		RenderTextOnScreen(meshList[GEO_TEXT], "Find somewhere else to sleep!", Color(1, 1, 1), 2, 1, 5);
		modelStack.PopMatrix();
	}
	//TANK
	modelStack.PushMatrix();
	...
	//BOMBS
	modelStack.PushMatrix();
	modelStack.Translate(-700, -3, -700);
	modelStack.Scale(15, 15, 15);
	RenderMesh(meshList[GEO_BOMB], false);
	modelStack.PopMatrix();
	modelStack.PushMatrix();
	modelStack.Translate(-700, -3, -650);
	modelStack.Rotate(90, 0, 1, 0);
	modelStack.Scale(15, 15, 15);
	RenderMesh(meshList[GEO_BOMB], false);
	modelStack.PopMatrix();
	modelStack.PushMatrix();
	modelStack.Translate(-660, -3, -650);
	modelStack.Rotate(90, 0, 1, 0);
	modelStack.Scale(15, 15, 15);
	RenderMesh(meshList[GEO_BOMB], false);
	modelStack.PopMatrix();
	...
	{
		modelStack.PushMatrix();
		RenderTextOnScreen(meshList[GEO_TEXT], "Press E to ignite the TNT", Color(1, 1, 1), 3, 1, 5);
		modelStack.PopMatrix();
	}
	if (elaspeTime < bombTime && triggeredBomb == true)
	{
		...
		ss << "You have " << bombTime - elaspeTime << " To Run";

		modelStack.PushMatrix();
		RenderTextOnScreen(meshList[GEO_TEXT], ss.str(), Color(1, 1, 1), 3, 1, 3);
		modelStack.PopMatrix();
	}
	if (elaspeTime > bombTime && triggeredBomb == true)
	{
		modelStack.PushMatrix();
		RenderTextOnScreen(meshList[GEO_TEXT], "You are dead.", Color(1, 1, 1), 5, 1, 3);
		modelStack.PopMatrix();

		modelStack.PushMatrix();
		modelStack.Translate(-675, 0, -660);
	...
	//WATCHTOWERS
	modelStack.PushMatrix();
	modelStack.Translate(720, 1, 720);
	modelStack.Rotate(0, 0, 1, 0);
	modelStack.Scale(30, 30, 30);
	RenderMesh(meshList[GEO_WATCHTOWER], false);
	modelStack.PopMatrix();
	modelStack.PushMatrix();
	modelStack.Translate(720, 1, -720);
	modelStack.Rotate(0, 0, 1, 0);
	...
	RenderMesh(meshList[GEO_WATCHWALL], false);
	modelStack.PopMatrix();

	//WATCH TOWER INTERACTIONS AND TEXTS
	if (nearTower == true)
	{
		modelStack.PushMatrix();
		RenderTextOnScreen(meshList[GEO_TEXT], "Press E to enter watchtower", Color(1, 1, 1), 2.5, 1, 5);
		modelStack.PopMatrix();
	}
	if (inTower == true || inTower2 == true || inTower3 == true)
	{
		modelStack.PushMatrix();
		RenderTextOnScreen(meshList[GEO_TEXT], "'Q' To exit watchtower", Color(1, 1, 1), 2, 1, 5);
		RenderTextOnScreen(meshList[GEO_TEXT], "'E'To toggle watchtower lights", Color(1, 1, 1), 2, 1, 2);
		modelStack.PopMatrix();
	}
	...
	//Guy
	modelStack.PushMatrix();
	modelStack.Translate(guyPosX, -9, guyPosZ);
	modelStack.Scale(5, 5, 5);
	RenderMesh(meshList[GEO_GUY], false);
	modelStack.PopMatrix();
	...
	{
		modelStack.PushMatrix();
		RenderTextOnScreen(meshList[GEO_TEXT], "Good morning, Sir", Color(1, 1, 1), 3, 1, 3);
		RenderTextOnScreen(meshList[GEO_TEXT], "'E' to control", Color(1, 1, 1), 3, 1, 2);
		modelStack.PopMatrix();
	}
	if (controlGuy == true)
	{
		nearGuy = false;
		modelStack.PushMatrix();
		RenderTextOnScreen(meshList[GEO_TEXT], "'Q' to stop", Color(1, 1, 1), 3, 1, 2);
		modelStack.PopMatrix();
	}
...
void SceneMorn::RenderMesh(Mesh *mesh, bool enableLight)
{
...
void SceneMorn::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void SceneMorn::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
