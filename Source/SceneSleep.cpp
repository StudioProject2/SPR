...
void SceneSleep::Update(double dt)
{
	elaspeTime += dt;
	deltaTime = dt;

	if (elaspeTime > bounceTime && isMorn == false)
	{
		sceneChange = 2;
	}
	else if (elaspeTime > bounceTime && isMorn == true)
	{
		sceneChange = 0;
	}
	camera.Update(dt);
...
void SceneSleep::Render()
{
	...
	RenderTextOnScreen(meshList[GEO_TEXT], "Sleeping...", Color(1, 1, 1), 3, 7, 10);
...
void SceneSleep::RenderMesh(Mesh *mesh, bool enableLight)
{
...
void SceneSleep::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void SceneSleep::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
